    ...
    public class ConfigurationDefaults
    {
        ...
        internal ConfigurationDefaults(string directory, string configFile)
        {
            try
            {
                ...
                {
                    _settingsManager = new Settings(new SettingsFile(directory, configFile));
                }
            }
        ...
        public IEnumerable<PackageSource> DefaultPackageSources
        {
            get
            {
                if (_defaultPackageSources == null)
                {
                    ...
                    var disabledPackageSources = _settingsManager.Sections[ConfigurationConstants.DisabledPackageSources]?.Children.Select(c => c as AddElement).Where(a => a != null);
                    var packageSources = _settingsManager.Sections[ConfigurationConstants.PackageSources]?.Children.Select(c => c as SourceElement).Where(a => a != null);
                    ...
                    foreach (var settingValue in packageSources)
                    {
                        ...
                        _defaultPackageSources.Add(new PackageSource(settingValue.Value,
                            settingValue.Key,
                            isEnabled: !disabledPackageSources.Any(p => p.Key.Equals(settingValue.Key, StringComparison.CurrentCultureIgnoreCase)),
                            isOfficial: true));
                    }
                }
        ...
        public string DefaultPackageRestoreConsent => SettingsUtility.GetValueForAddElement(_settingsManager, ConfigurationConstants.PackageRestore, ConfigurationConstants.Enabled);
    }
}
