...
namespace NuGet.Commands
{
    ...
    internal static class CommandRunnerUtility
    {
        ...
        public static string GetApiKey(ISettings settings, string endpoint, string source, string defaultApiKey, bool isSymbolApiKey)
        {
            ...
            var apiKey = SettingsUtility.GetDecryptedValueForAddElement(settings, ConfigurationConstants.ApiKeys, endpoint);

            // if not found try finding it by source url
            apiKey = apiKey ?? SettingsUtility.GetDecryptedValueForAddElement(settings, ConfigurationConstants.ApiKeys, source);
                ...
                && source.IndexOf(NuGetConstants.NuGetHostName, StringComparison.OrdinalIgnoreCase) >= 0)
            {
                ...
                apiKey = SettingsUtility.GetDecryptedValueForAddElement(settings, ConfigurationConstants.ApiKeys, defaultConfigKey);
            }
